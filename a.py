...
list=['Ha.jpg','HaHa.jpg','LMAO.jpg','XD.jpg']
iddict = {}
image  = cv2.imread("download.png")
imnew=cv2.resize(image,(1754,1240))
def arucoid(img):
...
for i in color:
    ...
    if i== 'black':
        bid = 3
cid = {'green':gid,'orange':oid,'white':wid,'black':bid}
        ...
        if aspectratio >=0.95 and aspectratio<=1.05:
            ...
            for i in color.keys():
                ...
                if (d==img[c[1],c[0]]).any():
                    ...
                    cv2.imshow('abc',f)
                    cv2.waitKey(1000)
                    imnew1 = cv2.addWeighted(imnew, 0.5, img, 0.5,0)
                    imnew2 = cv2.resize(imnew1,(0,0),fx = 0.5, fy = 0.5)
                    cv2.imshow('overlapped image',imnew2)
